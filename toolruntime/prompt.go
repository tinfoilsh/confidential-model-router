...
const (
	...
	toolEconomyInstructions    = "Reuse retrieved evidence only when it directly supports the current question. Previous assistant answers and plans to search are not retrieved evidence. When the user requests a new search, new sources, or verification, call the tools rather than describing a search or guessing its results. Saved excerpts from earlier turns are partial evidence, not a fresh lookup; cite their exact URLs with markdown links, not numbered cursors from earlier turns. If the available evidence is insufficient or a tool fails, state what you could not verify instead of inventing sources or claims. Avoid repeating equivalent searches or fetches once the question is supported or further attempts are unlikely to help."
