...
Chat Completions clients opting into `X-Tinfoil-Events: web_search` receive an optional
...
This saved-evidence transport is specific to Chat Completions. It does not add
fetch evidence to Responses API `open_page` output items.
...
truncate or compress source text. Router search metadata drops results without
URLs and deduplicates results by URL; fetch metadata drops pages without URLs.
Any limits already applied by the underlying search/fetch service still apply.
...
compaction, or prove that a model's claims follow from its citations.
