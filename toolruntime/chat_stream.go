...
func (s *chatStreamer) pumpUpstream(reader *sseReader) (chatIterationResult, error) {
	...
	// including the early returns below (client disconnect, upstream
	// error), so an interrupted turn is still billed for the tokens it
	// streamed. With continuous_usage_stats each chunk carries cumulative
	// usage, so the latest snapshot is the correct amount to fold.
	defer func() {
		if result.usage != nil {
...
// tool-loop turn. continuous_usage_stats makes vLLM attach cumulative
// usage to every chunk instead of only the final one, so a turn that is
// interrupted mid-stream still has usage to account for. The router never
// forwards upstream usage chunks to the client, so this is invisible to
// callers.
