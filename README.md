...
The existing worker re-attests all configured backends at startup and every five minutes by default (`REFRESH_INTERVAL` / `-r`), with at most four verifications in flight. A failed configuration fetch still re-attests the last known targets. Verification runs outside routing locks and does not run on the inference request path.
