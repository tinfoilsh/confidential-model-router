...
Opaque API keys use a local cached route context, keyed by credential and resolved served model, before file conversion or tool execution. Inference never waits for the control plane: each external request attempts one background `POST /api/shim/route-context` call to account for the request and update the cache for subsequent requests. This includes cached rejections so callers can recover. The control plane owns per-account/model shared RPM and lazy TPM policy: cached `rejected` returns 429 with a request/token-specific message and `Retry-After`, `demote` applies priority 1 on supported JSON endpoints, and `exempt` preserves configured priority and bypasses backend overload shedding. Cached credential/payment verdicts retain 401/403/402 and per-key quota verdicts retain 429.

The LRU cache has a 50 MiB accounting budget with conservative allowances for entry and string overhead; keys contain hashes rather than raw credentials. A cache miss uses the shared pool with default priority and no quota decision, while downstream shims still authenticate the request. Organization reservations and configured priority become available after the first successful refresh. Entries do not expire during outages: timeouts, transport errors, non-denial statuses, and unreadable decisions leave the cached status unchanged, including rejections. Refreshes survive request cancellation, have a 500 ms timeout, and are never retried. At most 256 refreshes run concurrently; at capacity, requests still use the cache and skip the refresh. Failed or skipped refreshes are counted in `router_route_context_lookup_failures_total` (`reason="busy"` for capacity). Older in-flight lookups cannot overwrite a newer cached result.

Bearers shaped as typed `at+jwt` access tokens skip route-context and have no local RPM limits. This classification bypasses only quota admission, not authentication. Billing, first-party safeguards, and delegated service credentials still apply. Internal tool/file dispatches reuse the request context without another admission. Input-token counting uses only the synchronous model-less metadata lookup (also skipped for JWTs), never rate admission. Runtime YAML carries no rate-limit policy (a `rate_limit` key is ignored); backend overload protection remains local.

The supported inference ingress is the router. Its first-hop shim authenticates only `/metrics`, as configured in `tinfoil-config.yml`; downstream model and tool shims verify inference JWT signatures and claims before serving. A forged JWT can match the router's classifier and skip quota lookup but must still be rejected by the downstream shim without obtaining inference. Classification does not establish that a token is already verified. Direct backend access is not a supported ingress and its quota-bypass implications are a separate deployment concern. Client-supplied request IDs never bypass admission. Only known inference JSON schemas are rewritten for priority; multipart, compressed, and opaque bodies on other endpoints retain their wire representation. The model is selected from the request body, multipart form, realtime query parameters, or an endpoint-specific default; the request host is not consulted. There is no implicit tool-server selection for public `/mcp` requests. Built-in tools select their configured MCP server internally.
