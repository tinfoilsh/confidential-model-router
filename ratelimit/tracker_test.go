...
func TestRecordCounts(t *testing.T) {
	...
	for i := int64(1); i <= 4; i++ {
		if got := tracker.Record("key1", "model1", 0, minute, minute); got.Count != i {
...
func TestDifferentKeysAreIndependent(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		tracker.Record("key1", "model1", 10, minute, minute)
	}

	if got := tracker.Record("key1", "model1", 10, minute, minute); got.Count != 6 || got.Tokens != 60 {
		t.Fatalf("key1/model1 expected count 6 tokens 60, got %d/%d", got.Count, got.Tokens)
	}
	if got := tracker.Record("key2", "model1", 10, minute, minute); got.Count != 1 || got.Tokens != 10 {
		t.Fatalf("key2/model1 expected count 1 tokens 10, got %d/%d", got.Count, got.Tokens)
	}
	if got := tracker.Record("key1", "model2", 10, minute, minute); got.Count != 1 || got.Tokens != 10 {
...
func TestWindowReset(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		tracker.Record("key1", "model1", 100, minute, minute)
	}
	if got := tracker.Record("key1", "model1", 100, minute, minute); got.Count != 6 || got.Tokens != 600 {
	...
	if got := tracker.Record("key1", "model1", 100, minute, minute); got.Count != 1 || got.Tokens != 100 {
...
func TestIndependentWindows(t *testing.T) {
	...
	tracker.Record("key1", "model1", 100, minute, 5*minute)
	tracker.Record("key1", "model1", 100, minute, 5*minute)

	// Next minute: count window rolls, token window does not
	now = time.Date(2025, 1, 1, 10, 31, 0, 0, time.UTC)
	got := tracker.Record("key1", "model1", 100, minute, 5*minute)
	...
	if got := tracker.Record("key1", "model1", 100, minute, 5*minute); got.Tokens != 100 {
...
func TestResetDuration(t *testing.T) {
	...
	if got := tracker.Record("key1", "model1", 0, minute, minute); got.CountResetIn != 45*time.Second {
		t.Fatalf("expected reset in 45s, got %v", got.CountResetIn)
	}

	now = time.Date(2025, 1, 1, 10, 30, 59, 5e8, time.UTC)
	if got := tracker.Record("key1", "model1", 0, minute, minute); got.CountResetIn != 500*time.Millisecond {
		t.Fatalf("expected reset in 500ms, got %v", got.CountResetIn)
	}

	// A fresh window has the full minute remaining
	now = time.Date(2025, 1, 1, 10, 31, 0, 0, time.UTC)
	if got := tracker.Record("key1", "model1", 0, minute, minute); got.CountResetIn != time.Minute {
...
func TestEmptyAPIKeyIgnored(t *testing.T) {
	...
	if got := tracker.Record("", "model1", 50, minute, minute); got != (Totals{}) {
...
func TestRefundTokens(t *testing.T) {
	...
	charged := tracker.Record("key1", "model1", 1000, minute, minute)
	...
	if got := tracker.Record("key1", "model1", 0, minute, minute); got.Tokens != 100 {
...
func TestRefundClampsAtZero(t *testing.T) {
	...
	charged := tracker.Record("key1", "model1", 100, minute, minute)
	tracker.RefundTokens("key1", "model1", 500, charged.TokensWindowStart)
	if got := tracker.Record("key1", "model1", 0, minute, minute); got.Tokens != 0 {
...
func TestRefundAfterWindowRollIsDropped(t *testing.T) {
	...
	now = time.Date(2025, 1, 1, 10, 31, 0, 0, time.UTC)
	tracker.Record("key1", "model1", 300, minute, minute)
	...
	if got := tracker.Record("key1", "model1", 0, minute, minute); got.Tokens != 300 {
...
func TestCleanup(t *testing.T) {
	...
	tracker.Record("key1", "model1", 0, minute, minute)

	// Advance 3 minutes and trigger cleanup via Record
	now = time.Date(2025, 1, 1, 10, 3, 1, 0, time.UTC)
	tracker.Record("key2", "model1", 0, minute, minute) // triggers cleanup
...
func TestCleanupSparesLongTokenWindows(t *testing.T) {
	...
	tracker.Record("key1", "model1", 500, minute, 10*minute)

	// 3 minutes on: the count window is stale but the token window is live
	now = time.Date(2025, 1, 1, 10, 3, 1, 0, time.UTC)
	tracker.Record("key2", "model1", 0, minute, minute) // triggers cleanup

	if got := tracker.Record("key1", "model1", 0, minute, 10*minute); got.Tokens != 500 {
...
func TestConcurrentAccess(t *testing.T) {
	...
	for i := 0; i < 100; i++ {
		...
		go func() {
			defer wg.Done()
			tracker.Record("key1", "model1", 7, minute, minute)
		}()
	}
	wg.Wait()

	if got := tracker.Record("key1", "model1", 0, minute, minute); got.Count != 101 || got.Tokens != 700 {
